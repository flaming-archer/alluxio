...
public class LocalAuthenticator implements Authenticator {
  ...
  private static final Logger LOG =
          LoggerFactory.getLogger(LocalAuthenticator.class);
  ...
  public void init(AlluxioConfiguration conf) {
    ...
    //mPasswordServiceUrl = "https://datasuite.staging.shopee.io/ram/api/v1/developer/sensitive/hadoopAccount/pwd/";
    //mPasswordServiceToken = "hjdhfkuyrkdsglasj37ljdsh71oapc4zjfn8ks27tdva8q1jf0h98jfhzmdyhjda";
    mPasswordServiceUrl =
        conf.get(PropertyKey.S3_REST_AUTHENTICATOR_LOCAL_PASSWORD_SERVICE_URL);
    if (mPasswordServiceUrl == null || mPasswordServiceUrl.isEmpty()) {
      throw new RuntimeException("LocalCheckAuthenticator Password Service URL not specified");
    }
    mPasswordServiceUrl = formatURL(mPasswordServiceUrl);

    mPasswordServiceToken =
        conf.get(PropertyKey.S3_REST_AUTHENTICATOR_LOCAL_PASSWORD_SERVICE_TOKEN);
    if (mPasswordServiceToken == null || mPasswordServiceToken.isEmpty()) {
      throw new RuntimeException("LocalCheckAuthenticator Password Service Token not specified");
    }

    mCacheSize = conf.getInt(PropertyKey.S3_REST_AUTHENTICATOR_LOCAL_CACHE_SIZE);
    mRefreshAfterWriteMin =
        conf.getInt(PropertyKey.S3_REST_AUTHENTICATOR_LOCAL_CACHE_REFRESH_TIMEOUT);
    mExpireAfterWriteMin =
        conf.getInt(PropertyKey.S3_REST_AUTHENTICATOR_LOCAL_CACHE_EXPIRE_TIMEOUT);
  ...
  private String getPassword(String userName) {
    ...
    try {
      ...
      if (responseCode != HttpURLConnection.HTTP_OK) {
        ...
        //throw new AccessControlException(response.toString());
      } else {
        String readLine;
